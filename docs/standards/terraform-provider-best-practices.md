...
1. **One problem domain.** This provider manages PowerDNS. Authoritative Server
   and Recursor are both in scope because they are one product family with one
   credential model; dnsdist is a documented open question (ADR 0006).
...
list. Half the inherited resources accept configuration the *server* then
rejects, so the failure surfaces at `apply` instead of `plan`
(capability map `CM-04` §5).
...
- Registry publishing needs an OSI licence (MPL-2.0, inherited), a valid
