...
Two of these are not hypothetical: `strings.Split` on a host-port string and
decoding a body without checking the status are both live defects in the
inherited code (`AUDIT-01`).

## 4. Mandatory standards

| Area | Standard |
|---|---|
| Module path | `github.com/dantte-lp/terraform-provider-powerdns`. |
